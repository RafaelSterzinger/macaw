...
class MACAW(object):
    def __init__(self, args: argparse.Namespace,
        ...
        self._is_discrete = type(env.action_space) is gym.spaces.discrete.Discrete
        ...
        self._adaptation_policy = MLP([self._observation_dim + goal_dim] +
                                      ...
                                      [self._action_dim],
                                      final_activation=torch.tanh if not self._is_discrete else lambda x: x,
                ...
                with torch.no_grad():
                    ...
                    if sample_mode:
                        if self._is_discrete:
                            action = torch.argmax(torch.softmax(mu, 0))
                        else:
                            action = mu
                    else:
                    ...
                    if self._is_discrete:
                        action = action.flatten().cpu().numpy()[0]
                    else:
